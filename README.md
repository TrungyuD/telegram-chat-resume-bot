...
- **Per-User Settings** — Each user configures their own model and effort level
...
| `ADMIN_TELEGRAM_IDS`             | _(required)_        | Comma-separated admin Telegram user IDs                 |
| `ALLOWED_WORKING_DIRS`           | _(empty = any)_     | Comma-separated allowed base directories for `/project` |
| `CLAUDE_DEFAULT_MODEL`           | `claude-sonnet-4-6` | Default Claude model                                    |
| `CLAUDE_DEFAULT_EFFORT`          | `medium`            | Default reasoning effort: `low`, `medium`, `high`       |
...
| `ADMIN_API_KEY`                  | _(empty)_           | API key for admin dashboard endpoints                   |
...
| Command           | Description                                             |
| ----------------- | ------------------------------------------------------- |
| _(any text)_      | Chat with Claude in full mode (all tools enabled)       |
...
| `/effort [level]` | `low`, `medium`, `high`   | Set reasoning effort level                        |
...
| `GET`  | `/ws`                       | API Key | WebSocket for real-time events |
...
If `ADMIN_API_KEY` is empty, dashboard admin endpoints (`/api/*`) and `/ws` return `503`.
...
| Biến                             | Mặc Định            | Mô Tả                                               |
| -------------------------------- | ------------------- | --------------------------------------------------- |
| `TELEGRAM_BOT_TOKEN`             | _(bắt buộc)_        | Token bot từ @BotFather                             |
| `ADMIN_TELEGRAM_IDS`             | _(bắt buộc)_        | Danh sách ID Telegram admin, cách nhau bởi dấu phẩy |
| `ALLOWED_WORKING_DIRS`           | _(trống = tất cả)_  | Thư mục làm việc cho phép, cách nhau bởi dấu phẩy   |
| `CLAUDE_DEFAULT_MODEL`           | `claude-sonnet-4-6` | Model Claude mặc định                               |
| `CLAUDE_DEFAULT_EFFORT`          | `medium`            | Mức suy luận mặc định: `low`, `medium`, `high`      |
...
| `ADMIN_API_KEY`                  | _(trống)_           | API key cho endpoint quản trị                       |
...
| Lệnh                   | Mô Tả                                                            |
| ---------------------- | ---------------------------------------------------------------- |
| _(gửi văn bản bất kỳ)_ | Chat với Claude chế độ đầy đủ (tất cả công cụ)                   |
...
| `/effort [mức]` | `low`, `medium`, `high`   | Đặt mức độ suy luận                      |
...
| `GET`       | `/ws`                       | API Key  | WebSocket cho sự kiện thời gian thực |
...
Nếu `ADMIN_API_KEY` để trống, các endpoint quản trị dashboard (`/api/*`) và `/ws` sẽ trả về `503`.
...
_Built with Go, powered by Claude AI._
