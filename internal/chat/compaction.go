...
func CompactSessionIfNeeded(cfg *config.GlobalConfig, telegramID, sessionID string) error {
	if !cfg.CompactEnabled {
		return nil
	}

	count, err := sessions.GetSessionMessageCount(telegramID, sessionID)
	if err != nil || count <= cfg.CompactThreshold {
		return err
	}

	messages, err := sessions.GetSessionMessages(telegramID, sessionID)
	if err != nil {
		return err
	}
	...
	sessionPath := sessions.GetSessionPath(telegramID, sessionID)
	meta, _, err := storage.ParseSessionMD(sessionPath)
	if err != nil {
		return err
	}
	...
	if err := storage.WriteSessionMD(sessionPath, meta, recentMessages); err != nil {
		return err
	}
