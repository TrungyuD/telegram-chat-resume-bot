...
// SetDataDir changes the data directory. Use in tests only.
...
// LockFile acquires a per-path mutex and returns an unlock function.
...
type SessionMeta struct {
	...
	ClaudeSessionID   string `yaml:"claude_session_id,omitempty" json:"claude_session_id,omitempty"`
...
func parseMessages(body string) []SessionMessage {
	...
	messages := make([]SessionMessage, 0, len(locs))
...
func AppendSessionMessage(path, role, content string) error {
